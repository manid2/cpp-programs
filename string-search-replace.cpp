...
{
	string str_o;
	str_o = string(c_style::find_n_replace(str_i.c_str(), str_f.c_str(),
	                                       str_r.c_str()));
	return str_o;
}
                           ...
                           const char *str_r)
{
	...
	if (0 != sub_pos) {
		...
		do {
			str_append(str_i + str_i_cur_pos,
			           str_o + str_o_cur_pos,
			...
			sub_pos = str_find(str_i + str_i_cur_pos, str_f,
			                   str_i_consumed_len);
		} while (0 != sub_pos);
	} else {
		str_o = str_i;
